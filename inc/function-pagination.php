...
function canhcam_pagination($custom_query = null, $args = array()) {
    ...
    $get_link = function($page) use ($args) {
        return $args['is_ajax'] ? 'javascript:void(0);' : esc_url(get_pagenum_link($page));
    };
    ...
    if ($args['show_prev_next'] && $current_page > 1) {
        ...
        $output .= '<li class="prev"><a href="' . $get_link($prev_page) . '" data-page="' . $prev_page . '" aria-label="' . esc_attr__('Trang trước', 'canhcamtheme') . '">' . $args['prev_text'] . '</a></li>';
    }
    
    // First page + dots
    if ($start_page > 1) {
        $output .= '<li><a href="' . $get_link(1) . '" data-page="1">1</a></li>';
        if ($start_page > 2) {
    ...
    for ($i = $start_page; $i <= $end_page; $i++) {
        if ($i == $current_page) {
            $output .= '<li class="active"><span data-page="' . $i . '">' . $i . '</span></li>';
        } else {
            $output .= '<li><a href="' . $get_link($i) . '" data-page="' . $i . '">' . $i . '</a></li>';
        }
    }
    ...
    if ($end_page < $total_pages) {
        ...
        $output .= '<li><a href="' . $get_link($total_pages) . '" data-page="' . $total_pages . '">' . $total_pages . '</a></li>';
    }
    
    // Next page link
    if ($args['show_prev_next'] && $current_page < $total_pages) {
        $next_page = $current_page + 1;
        $output .= '<li class="next"><a href="' . $get_link($next_page) . '" data-page="' . $next_page . '" aria-label="' . esc_attr__('Trang sau', 'canhcamtheme') . '">' . $args['next_text'] . '</a></li>';
    }
